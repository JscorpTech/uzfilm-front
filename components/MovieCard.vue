  ...
  <div class="movie-card">
    <div class="movie-card__img">
      ...
      <NuxtLink :to="movieLink">
        <img class="img" :src="movieThumbnail" alt="" />
      ...
      <div class="movie-card__rating">
        ...
        >
          <g clip-path="url(#clip0_288_1127)">
            ...
            />
            <path
              d="M15 23.0001L15 9.00012C15 8.73491 14.8946 8.48055 14.7071 8.29302C14.5196 8.10548 14.2652 8.00012 14 8.00012C13.7348 8.00012 13.4804 8.10548 13.2929 8.29302C13.1054 8.48055 13 8.73491L13 23.0001C13 23.2653 13.1054 23.5197 13.2929 23.7072C13.4804 23.8948 13.7348 24.0001 14 24.0001C14.2652 24.0001 14.5196 23.8948 14.7071 23.7072C14.8946 23.5197 15 23.2653 15 23.0001Z"
              fill="#7A7A7A"
            />
            <path
              d="M5 23.0001L5 9.00012C5 8.73491 4.89464 8.48055 4.70711 8.29302C4.51957 8.10548 4.26521 8.00012 3.99999 8.00012C3.73478 8.00012 3.48042 8.10548 3.29288 8.29302C3.10535 8.48055 2.99999 8.73491L2.99999 23.0001C2.99999 23.2653 3.10534 23.5197 3.29288 23.7072C3.48042 23.8948 3.73478 24.0001 3.99999 24.0001C4.26521 24.0001 4.51957 23.8948 4.70711 23.7072C4.89464 23.5197 5 23.2653 5 23.0001Z"
              fill="#7A7A7A"
            />
            <path
    ...
    <div class="movie-card__badge-wrapper">
      <span class="movie-card__badge-quality">{{
        props.movie?.is_free ? "BEPUL" : "OBUNA"
      }}</span>
      <span class="movie-card__badge-new" v-if="props.movie?.is_premiere"
        >NEW</span
      >
    </div>

    <div class="movie-card__text-wrapper">
      <NuxtLink class="movie-card__title" :to="movieLink">
        {{ props.movie?.title }}
      </NuxtLink>
...
async function addFvrt() {
  ...
  const isFavorite = activeSave.value;
...
function yearGet(y: string | number) {
  const dateObject = new Date(y);
  return dateObject.getFullYear();
}

const activeSave = computed(() => {
  if (!authStore.token || !store.savedMovies?.data?.content) {
    return false;
  }

  const savedItems = toRaw(store.savedMovies.data.content);
  return savedItems.some((elem: any) => elem.id === props.movie.id);
});

const movieLink = computed(() => {
  if (props.movie?.is_mobile_only) {
    return "/only-on-mobile";
  } else if (!authStore.token) {
    return "/auth/login/";
  } else if (props.movie?.is_movie) {
    return `/movie/${props.movie.slug}/`;
  } else {
    return `/series/${props.movie.id}?page=1`;
  }
});

const movieThumbnail = computed(() => {
  const baseUrl = "https://gateway.pandatv.uz";
  return props.movie?.thumbnail_image.includes(store.baseUrl)
    ? props.movie.thumbnail_image
    : `${baseUrl}${props.movie.thumbnail_image}`;
});
</script>
